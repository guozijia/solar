<template>
  <div class="my_table">
    <el-table :header-cell-style="{background:'#f8f9fa'}" size="small" :data="list" style="width: 100%;">
      ...
      <el-table-column label="本周咨询次数" align="center">
        <template slot-scope="scope">
          {{ scope.row.monthAdd }}
        </template>
      </el-table-column>
      <el-table-column label="本周已解决问题" align="center">
        <template slot-scope="scope">
          {{ scope.row.stationTotal }}
        </template>
      </el-table-column>
      <el-table-column label="平均解决时间" align="center">
        <template slot-scope="scope">
          {{ scope.row.stationTotal }}
        </template>
      </el-table-column>
      <el-table-column label="累计咨询次数" align="center">
        <template slot-scope="scope">
          {{ scope.row.stationTotal }}
        </template>
      </el-table-column>
