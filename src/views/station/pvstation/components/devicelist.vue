      ...
      <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button-group>
            ...
            <el-tooltip class="item" effect="dark" content="IV曲线" placement="top">

              <el-button size="mini" icon="el-icon-cpu" @click="handleLink(row.code)" />
            </el-tooltip>
          </el-button-group>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleLink(code) {
      this.$router.push({ path: '/Operation/diagnosis', query: { code }})
    },
    sortByID(order) {
      if (order === 'ascending') {
